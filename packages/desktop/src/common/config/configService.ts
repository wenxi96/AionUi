...
function getRendererBackendPort(): number {
  const dynamicPort = window.__backendPortState?.getPort?.();
  if (dynamicPort && dynamicPort > 0) return dynamicPort;
  return window.__backendPort || 0;
}

function getBaseUrl(): string {
  // WebUI browser mode: no preload, fetch same-origin so web-host's
  // static-server reverse-proxies /api/* to the backend.
  if (typeof window !== 'undefined' && typeof document !== 'undefined' && !getRendererBackendPort()) {
    return '';
  }
  const port = typeof window !== 'undefined' ? getRendererBackendPort() || 13400 : 13400;
